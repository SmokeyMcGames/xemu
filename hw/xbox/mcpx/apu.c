 ...
 * Copyright (c) 2019-2020 Matt Borgerson
 *
...
static bool should_mute_voice(int v);
static uint32_t voice_get_mask(MCPXAPUState *d, unsigned int voice_handle,
                               hwaddr offset, uint32_t mask);
static void voice_set_mask(MCPXAPUState *d, unsigned int voice_handle,
...
static void ep_write(void *opaque, hwaddr addr, uint64_t val,
                     unsigned int size);
static float step_envelope(MCPXAPUState *d, unsigned int v, uint32_t reg_0,
...
static void set_notify_status(MCPXAPUState *d, uint32_t v, int notifier,
                              int status);
static long resampler_callback(void *cb_data, float **data);
static int resample_voice(MCPXAPUState *d, int v, float samples[][2],
                          int requested_num, float rate);
static void reset_filters(MCPXAPUState *d, int v);
static float clampf(float v, float min, float max);
static float attenuate(uint16_t vol);
static void process_voice(MCPXAPUState *d,
                          float mixbins[NUM_MIXBINS][NUM_SAMPLES_PER_FRAME],
                          uint32_t v);
static int get_samples(MCPXAPUState *d, uint32_t v, float samples[][2],
...
void mcpx_apu_debug_isolate_voice(int v)
{
...
bool mcpx_apu_debug_is_muted(int v)
{
    return g_dbg_muted_voices[v / 64] & (1LL << (v % 64));
}

void mcpx_apu_debug_toggle_mute(int v)
{
...
static void fe_method(MCPXAPUState *d, uint32_t method, uint32_t argument)
{
    ...
    case NV1BA0_PIO_VOICE_LOCK: {
        ...
        /* FIXME: This needs to stall the setup engine from processing this
         * voice.
         */
        break;
    }
    ...
    case NV1BA0_PIO_VOICE_ON:
        ...
        reset_filters(d, selected_handle);
        voice_set_mask(d, selected_handle, NV_PAVS_VOICE_PAR_STATE,
    ...
    case NV1BA0_PIO_VOICE_RELEASE: {
        ...
        // FIXME: Grab voice lock to prevent race cond
    ...
    case NV1BA0_PIO_VOICE_OFF:
        selected_handle = argument & NV1BA0_PIO_VOICE_OFF_HANDLE;
        voice_set_mask(d, selected_handle, NV_PAVS_VOICE_PAR_STATE,
                       NV_PAVS_VOICE_PAR_STATE_ACTIVE_VOICE, 0);
        // FIXME: Set SSLB_DONE if stream and current list is B?
        set_notify_status(d, selected_handle, MCPX_HW_NOTIFIER_SSLA_DONE,
...
static void vp_write(void *opaque, hwaddr addr, uint64_t val, unsigned int size)
{
    ...
    case NV1BA0_PIO_SET_SUBMIX_HEADROOM ...
        ...
        // qemu_mutex_lock(&d->lock); // FIXME: Possible starvation, finer locking
                           ...
                           uint32_t count_mask, uint32_t cur_mask)
{
    ...
    case NV_PAVS_VOICE_PAR_STATE_EFCUR_RELEASE: {
        ...
        float value = 0;
        if (count == 0) {
            // FIXME: Refactor below to a voice_off function
            ...
            // FIXME: Should move to a new state (force-release)?
        } else {
                              ...
                              int status)
{
    ...
    d->regs[NV_PAPU_ISTS] |= NV_PAPU_ISTS_FEVINTSTS;
    d->regs[NV_PAPU_ISTS] |= NV_PAPU_ISTS_FENINTSTS;
    d->set_irq = true;
}
                          ...
                          int requested_num, float rate)
{
    ...
    if (filter->resampler == NULL) {
         ...
         * so use it for now.
         */
        filter->resampler = src_callback_new(&resampler_callback,
                          ...
                          uint32_t v)
{
    ...
    for (int sample_count = 0; sample_count < NUM_SAMPLES_PER_FRAME;) {
        ...
        int count = resample_voice(d, v, &samples[sample_count],
                       ...
                       int num_samples_requested)
{
    ...
    if (stream) {
        ...
        if (count == 0) {
            ...
            if (!persist) {
                d->vp.ssl[v].ssl_index = 0; // FIXME: Check this...
            }
        ...
        if (adpcm) {
            ...
            if (adpcm_block_index != block_index) {
                ...
                if (stream) {
                    ...
                    int max_seg_byte =
                        ((seg_len >> 6) * samples_per_block * 9 * 4);
...
static void se_frame(MCPXAPUState *d)
{
    ...
    int num_bytes_free = fifo8_num_free(&d->vp_out_buf);
    ...
    for (int list = 0; list < 3; list++) {
        ...
        for (int i = 0; d->regs[current] != 0xFFFF; i++) {
            ...
            d->regs[next] = voice_get_mask(
                d, d->regs[current], NV_PAVS_VOICE_TAR_PITCH_LINK,
                NV_PAVS_VOICE_TAR_PITCH_LINK_NEXT_VOICE_HANDLE);
            if (!voice_get_mask(d, d->regs[current], NV_PAVS_VOICE_PAR_STATE,
                                NV_PAVS_VOICE_PAR_STATE_ACTIVE_VOICE)) {
    ...
    if ((d->ep_frame_div + 1) % 8 == 0) {
        ...
        fifo8_push_all(&d->vp_out_buf, (uint8_t *)d->apu_fifo_output,
...
static void mcpx_vp_out_cb(void *opaque, uint8_t *stream, int free_b)
{
    ...
    while (avail < free_b) {
        ...
        avail = fifo8_num_used(&s->vp_out_buf);
    ...
    while (to_copy > 0) {
        ...
        const uint8_t *samples =
            fifo8_pop_buf(&s->vp_out_buf, to_copy, &chunk_len);
...
void mcpx_apu_init(PCIBus *bus, int devfn, MemoryRegion *ram)
{
    ...
    fifo8_create(&d->vp_out_buf, 3 * (256 * 2 * 2));
